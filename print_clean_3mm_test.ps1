...
public class CleanLabelSpool {
 [StructLayout(LayoutKind.Sequential,CharSet=CharSet.Unicode)] public class Doc { public string name="Clean 3mm label test"; public string file=null; public string type="RAW"; }
...
$job = [CleanLabelSpool]::Send($printer.Name, ($lines -join "`r`n") + "`r`n")
Write-Output "Submitted clean gap-$GapMm job $job with $Copies labels."
